# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 14:55:08 2022

@author: dwinant
"""
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  3 16:37:45 2021
...
from utils_AM import Lin_View, get_laserprofile_dataloader
import os


# Uses CNN architecture of Rob Heylen from Flanders Make for AM Use Case

class AM_Enc3(nn.Module):
    """ Encoder - network architecture """
    def __init__(self, nChannels, args, cnn_kwargs):
        super(AM_Enc3, self).__init__()  # inheritance used here.
        self.args = args
        self.main = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size = 2, stride=2),
            nn.Flatten(),
            nn.Linear(6336, self.args.x_fdim2),
            )
...
class AM_Dec3(nn.Module):
    """ Decoder - network architecture """
    def __init__(self, nChannels, args, cnn_kwargs):
        super(AM_Dec3, self).__init__()
        self.args = args
        self.main = nn.Sequential(
              nn.Linear(self.args.x_fdim2, 6336),
              Lin_View(64,9,11),
...
class AM_RKM_Stiefel3(nn.Module):
    ...
    def __init__(self, ipVec_dim, args, nChannels=1, recon_loss=nn.MSELoss(reduction='sum'), ngpus=1):
        super(AM_RKM_Stiefel3, self).__init__()
        ...
        self.encoder = AM_Enc3(self.nChannels, self.args, self.cnn_kwargs)
        self.decoder = AM_Dec3(self.nChannels, self.args, self.cnn_kwargs)
...
def final_compute(model, args, ct, device=torch.device('cuda'), path_to_data = None):
    ...
    args.shuffle = False
    x, _, _ = get_laserprofile_dataloader(args, path_to_data)
